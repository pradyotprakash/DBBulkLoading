...
AM_SplitLeaf(fileDesc,pageBuf,pageNum,attrLength,recId,value,status,index,key,num_nodes)
...
AM_AddtoParent(fileDesc,pageNum,value,attrLength,buff_hits,num_nodes,buff_access)
int fileDesc;
...

{
 ...
 //printf("Buffer Accesses=> %d ,%p\n",(*buff_access), buff_access);
	...
	/* Get the parent node */
	errVal = PF_GetThisPage(fileDesc,pageNumber,&pageBuf,buff_hits);
	AM_Check;
	...
	else
	{
		...
		else
		{
			...
			errVal =  AM_AddtoParent(fileDesc,pageNum1,value,
						 attrLength,buff_hits,num_nodes,buff_access);
			AM_Check;
		}
	}
	return(AME_OK);
